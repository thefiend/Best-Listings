...
- **JPEGs (place photos):** max 800px wide, quality 80, progressive, mozjpeg via sharp. `fetch-place-photos.py` handles this automatically.
- **PNGs (OG/featured):** generated by `generate-og-image.ts` at correct size, no further compression needed.
- **Any other images:** run through sharp or equivalent before committing.
